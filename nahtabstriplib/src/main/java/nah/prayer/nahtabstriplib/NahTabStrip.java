...
package nah.prayer.nahtabstriplib;
...
public class NahTabStrip extends View implements ViewPager.OnPageChangeListener {

    private String TAG="nah";
    ...
    private final static int DEFAULT_STROP_SIDE_WEIGHT = 1;
    private final static boolean DEFAULT_STROP_FIT_TEXT = false;
    ...
    public NahTabStrip(final Context context) {
        this(context, null);
    }

    public NahTabStrip(final Context context, final AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public NahTabStrip(final Context context, final AttributeSet attrs, final int defStyleAttr) {
        ...
        final TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.NahTabStrip);
        try {
            setStripColor(
                    typedArray.getColor(R.styleable.NahTabStrip_nts_color, DEFAULT_STRIP_COLOR)
            );
            setTitleSize(
                    typedArray.getDimension(R.styleable.NahTabStrip_nts_size, DEFAULT_TITLE_SIZE)
            );
            setTitleBold(
                    typedArray.getBoolean(R.styleable.NahTabStrip_nts_bold, DEFAULT_TITLE_BOLD)
            );
            setStripWeight(
                    typedArray.getDimension(R.styleable.NahTabStrip_nts_weight, DEFAULT_STRIP_WEIGHT)
            );
            setStripSideWeight(
                    typedArray.getInt(R.styleable.NahTabStrip_nts_side_weight, DEFAULT_STROP_SIDE_WEIGHT)
            );
            setStripFitText(
                    typedArray.getBoolean(R.styleable.NahTabStrip_nts_fit_text, DEFAULT_STROP_FIT_TEXT)
            );
            setStripFactor(
                    typedArray.getFloat(R.styleable.NahTabStrip_nts_factor, DEFAULT_STRIP_FACTOR)
            );
            setStripType(
                    typedArray.getInt(R.styleable.NahTabStrip_nts_type, StripType.LINE_INDEX)
            );
            setStripGravity(
                    typedArray.getInt(R.styleable.NahTabStrip_nts_gravity, StripGravity.BOTTOM_INDEX)
            );

            setTypeface(typedArray.getString(R.styleable.NahTabStrip_nts_typeface));
            setInactiveColor(
                    typedArray.getColor(
                            R.styleable.NahTabStrip_nts_inactive_color, DEFAULT_INACTIVE_COLOR
                    )
            );
            setActiveColor(
                    typedArray.getColor(
                            R.styleable.NahTabStrip_nts_active_color, DEFAULT_ACTIVE_COLOR
                    )
            );
            setAnimationDuration(
                    typedArray.getInteger(
                            R.styleable.NahTabStrip_nts_animation_duration, DEFAULT_ANIMATION_DURATION
                    )
            );
            setCornersRadius(
                    typedArray.getDimension(
                            R.styleable.NahTabStrip_nts_corners_radius, DEFAULT_CORNER_RADIUS
                    )
            );
            ...
            try {
                final int titlesResId = typedArray.getResourceId(
                        R.styleable.NahTabStrip_nts_titles, 0
                );
    ...
    public void setStripSideWeight(final int weight) {
        mStripSideWeight = weight;
        postInvalidate();
    }
    ...
    public void setStripFitText(final boolean bool) {
        mStripFitText = bool;
    }
    ...
    public void setTabIndex(int tabIndex, boolean isForce) {
        ...
        mIndexTextSize = mTitlePaint.measureText(mTitles[index]);
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        float tmpLeft = mStripLeft - (mStripType == StripType.POINT ? mStripWeight * 0.5F : 0.0F);
        float tmpRight = mStripRight - (mStripType == StripType.POINT ? mStripWeight * 0.5F : 0.0F);
        float length = tmpRight-tmpLeft;

        float left;
        float right;
        if (mStripFitText) {
            left = ((tmpLeft + tmpRight) / 2) - mIndexTextSize / 2;
            right = ((tmpLeft + tmpRight) / 2) + mIndexTextSize / 2;
        } else {
            left = ((tmpLeft + tmpRight) / 2) - (length / mStripSideWeight) / 2;
            right = ((tmpLeft + tmpRight) / 2) + (length / mStripSideWeight) / 2;
        }
        ...
        for (int i = 0; i < mTitles.length; i++) {
            ...
            if(length<mTitlePaint.measureText(title)){
                mTitles[i] = title = title.substring(0,(int) Math.floor(title.length()/(mTitlePaint.measureText(title)/length)));
            }
